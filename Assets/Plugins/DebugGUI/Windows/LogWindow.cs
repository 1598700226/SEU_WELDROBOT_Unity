...
namespace WeavUtils
{
    public partial class LogWindow : DebugGUIWindow
    {
        ...
        protected override void OnGUI()
        {
            ...
            Vector2 position = DebugGUI.Settings.LogsPosition;

            var backgroundRect = new Rect(position, new Vector2(textSize.x, textSize.y));
            DrawRect(backgroundRect, DebugGUI.Settings.backgroundColor, Padding);
            // Draw a little bit extra for the draggable area
            DrawRect(new Rect(position.x, position.y, rect.width, rect.height), new Color(1, 1, 1, 0.05f), Padding);

            // Draw persistent logs
            DrawLabel(new Rect(position, textSize), persistentLogStr, Padding, textStyle);
            ...
            for (int i = transientLogs.Count - 1; i >= 0; i--)
            {
                ...
                var log = transientLogs[i];
                DrawLabel(position + Vector2.up * transientLogY, log.text, Padding, textStyle);
